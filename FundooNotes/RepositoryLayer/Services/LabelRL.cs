...
namespace RepositoryLayer.Services
{
    public class LabelRL : ILabelRL
    {
        ...
        public async Task<Label> GetLabel(int UserId, int NoteId)
        {
            try
            {
                return await fundooContext.Label.FirstOrDefaultAsync(x => x.UserId == UserId && x.NoteId == NoteId);
            }
            catch (Exception)
            {
        ...
        public async Task<List<Label>> GetallLabel(int UserId)
        {
            try
            {
                ...
                label = await fundooContext.Label.Where(x => x.UserId == UserId).ToListAsync();
                return label;
            }
            catch (Exception)
            {
